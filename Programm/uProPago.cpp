...
cMantSecPag::cMantSecPag() : cMantArchSec("MANTENIMIENTO de PAGOS", "PAGOS.DAT", &RegPago, sizeof(RegPago), 4, 1, 23, 80, ListadoConFiltro)
{
    if (mTodoCorrecto())
    {
...
//-------------------------------------------------------------------------
void cMantSecPag::AdicionaInformacion()
{
...
int cMantSecPag::ObtienePrestamo()
{
    ...
    else
    {
        ...
        else if (RegPrestamo.NumCuotas == RegPrestamo.NumPagos)
        {
...
int cMantSecPag::ClaveEncontrada()
{
    ...
    else // si es INGRESOS, clave autogenerada
        return 0;
}
//-------------------------------------------------------------------------
void cMantSecPag::MuestraInformacion()
{
    ...
    EscribeCad(40, 10, voCadena.ConBlancosIzq(FormatoNum(RegPrestamo.MontoDePres + RegPrestamo.MontoDelnt, 2), 10));
    ...
    EscribeCad(40, 13, voCadena.ConBlancosIzq(FonnatoNum(RegPrestamo.NumPagos, 0), 10));
    EscribeCad(40, 14, voCadena.ConBlancosIzq(FormatoNum(RegPrestamo.TotalAmort + RegPrestamo.TotalInt, 2), 10));

    EscribeCad(40, 16, voCadena.ConBlancosIzq(FormatoNum(RegPrestamo.MontoDePres + RegPrestamo.MontoDeInt - RegPrestamo.TotalAmort - RegPrestamo.TotalInt, 2), 10));
    EscribeCad(40, 18, FormatoFecha(RegPago.FechaDePago.dia, RegPago.FechaDePago.mes, RegPago.FechaDePago.anio));
    EscribeCad(40, 19, voCadena.ConBlancosIzq(FormatoNum(RegPago.ImpDePago, 2), 10));
    EscribeCad(40, 20, voCadena.ConBlancosIzq(FormatoNum(RegPago.ImpDelnt, 2), 10));
    EscribeCad(40, 21, voCadena.ConBlancosIzq(FonnatoNum(RegPago.ImpDeAmort, 2), 10));
}
//-------------------------------------------------------------------------
...
{
    // devuelve el contenido de la infonnación del registro a presentar
