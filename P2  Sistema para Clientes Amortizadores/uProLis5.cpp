...
int cListPagPre::InformacionMostrable(){
    ...
    return (RegPago.Estado && (RegPago.NumPrestamo == RegPrestamo.NumPrestamo));
}
...
void cListPagPre::MuestraDatosPrestammo(){
    cCadena voCadena;
    ...
    char apynomb[80];
    strcpy(apynomb, RegCliente.Apellidos);
    strcat(apynomb, " ");
    strcat(apynomb, RegCliente.Nombres);

    EscribeCad(20, 4, voCadena.SinBlancosDer(apynomb));
    EscribeCad(14, 5, FormatoFecha(RegPrestamo.FechaDePres.dia,
                                   RegPrestamo.FechaDePres.mes,
                                   RegPrestamo.FechaDePres.anio));
    EscribeCad(47, 5, reinterpret_cast<const char*>(AlineaDer(3, FormatoNum(RegPrestamo.NumPagos, 0))));
    EscribeCad(70, 5, reinterpret_cast<const char*>(AlineaDer(10, FormatoNum(RegPrestamo.Mensualidad, 2))));
    EscribeCad(47, 6, reinterpret_cast<const char*>(AlineaDer(3, FormatoNum(RegPrestamo.NumCuotas, 0))));

    EscribeCad(14, 6, FormatoNum(RegPrestamo.MontoDePres +
                                 RegPrestamo.MontoDeInt, 2));
    EscribeCad(70, 6, reinterpret_cast<const char*>(AlineaDer(10, FormatoNum(RegPrestamo.MontoDePres +
                                               RegPrestamo.MontoDeInt -
                                               RegPrestamo.TotalAmort -
                                               RegPrestamo.TotalInt, 2))));
}
...
void cListPagPre::Totaliza(){
    ...
    if(voTotImpDePago > 0){
        ...
        EscribeCad(19, 25, reinterpret_cast<const char*>(AlineaDer(10, FormatoNum(voTotImpDePago, 2))));
        EscribeCad(30, 25, reinterpret_cast<const char*>(AlineaDer(10, FormatoNum(voTotImpDeInt, 2))));
        EscribeCad(40, 25, reinterpret_cast<const char*>(AlineaDer(10, FormatoNum(voTotImpDeAmort, 2))));
    }
}
